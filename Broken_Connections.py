...
def DefineIntProps(my_mdb, cohesive_table, damage_init_table,
    ...
    my_mdb.ContactProperty('IntProp-1-Broken')
    my_mdb.interactionProperties['IntProp-1-Broken'].TangentialBehavior(
        formulation=PENALTY, directionality=ISOTROPIC, slipRateDependency=OFF, 
        pressureDependency=OFF, temperatureDependency=OFF, dependencies=0,
        table=tangential_table, shearStressLimit=None, maximumElasticSlip=FRACTION, 
        fraction=0.005, elasticSlipStiffness=None)
    my_mdb.interactionProperties['IntProp-1-Broken'].CohesiveBehavior(
        defaultPenalties=OFF, table= ((1, 1, 1),))
    my_mdb.interactionProperties['IntProp-1-Broken'].Damage(
        initTable=((1, 1, 1),),
        useEvolution=ON,
        evolutionType=ENERGY,
        evolTable=damage_evolution_table)
...
def DefineInteractions(my_mdb, flake_list, matrix_name, cut_flakes_names, cut_flakes_length, 
    ...
    for i in range(len(pairlist)):
        #there really isnt a binomial trial in the random package... sigh
        #I guess I will have to do it the hard way.
        test = random.uniform(0,1)
        if(test < percent_broken):
            propertytype = 'IntProp-1-Broken'
        else:
            propertytype = 'IntProp-1'
        temppair = (first_list[i], second_list[i], propertytype)
...
T_vector = 64.0
G_vector = 0.2
flake_cons_vector = 0.8
percent_broken_vector = [0.25, 0.50, 0.75]

for i in range(len(percent_broken_vector)):
    for j in range(5):
        GenerateGeometry(n_flake_y=12,
                         matrix_length=0.75,
                         aspect_ratio=75,
                         run_seed = 166798136041,
                         flake_concentration_x=flake_cons_vector,
                         cohesive_table = MakeTable(K_value, 3),
                         damage_init_table = MakeTable(T_vector, 3),
                         damage_evolution_table = MakeTable(G_vector, 1),
                         percent_broken = percent_broken_vector[i])
